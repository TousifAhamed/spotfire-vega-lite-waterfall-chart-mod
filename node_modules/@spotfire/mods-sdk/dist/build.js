...
import { ModType, debounce, formatIfPossible, getDirname, isParameterType, mkStdout, toTypeName, } from "./utils.js";
 ...
 */
async function createEnvFile({ manifestPath, envPath, ...quiet }) {
...
`;
        for (const param of script.parameters ?? []) {
            ...
            if (!param.type) {
                ...
                error(`Parameter '${param.name}' has an invalid type: '${param.type}'`);
            }
        }
    ...
    if (!hasFailed) {
        const formattedEnvFileContent = await formatIfPossible(envPath, envFileContent, true);
...
let ctx;
async function restartEsbuildImpl({ defaultConfig, esbuildConfigPath, outdir, debug, quiet, }) {
    try {
        if (ctx) {
            await ctx.dispose();
        }
...
function isAllowedEndpointPath(fileName) {
 ...
 * esbuild.
 */
export async function build({ outdir: outDir, src: srcDir, watch, debug, manifestPath, envPath, esbuildConfig, ...quiet }) {
...
async function getTypeFromManifest(manifestPath) {
    ...
    try {
        ...
        if (apiVersion < 2) {
            console.info(`'apiVersion' is less than 2.0, assuming visualization mod.`);
        ...
        if (type === "action") {
            ...
            console.error(`Unknown mod type: ${type}. Valid types are: 'action' and 'visualization'.`);
        }
    }
    catch (e) {
        console.error(`Failed to parse '${manifestPath}' as JSON. Exception: ${e}`);
    }
    return null;
}
async function getEsbuildOptions({ defaultConfig, esbuildConfigPath, outdir, debug, ...quiet }) {
    const stdout = mkStdout(quiet);
        ...
        try {
            ...
            if (typeof userConfig === "object") {
                ...
                for (const defaultConfigKey of Object.keys(defaultConfig)) {
                    ...
                    const oldVal = defaultConfig[defaultConfigKey];
                ...
                if (Object.keys(overrides).length > 0) {
                    ...
                    for (const [key, { oldVal, newVal }] of Object.entries(overrides)) {
...
async function buildActionMod({ absOutDir, srcDir, manifestPath, esbuildConfig: esbuildConfigPath, envPath, debug, watch, ...quiet }) {
    const stdout = mkStdout(quiet);
    const scriptsDir = path.join(srcDir, "scripts");
    if (!fse.existsSync(scriptsDir)) {
        throw new Error(`Cannot find 'scripts' folder in source directory, looked at '${scriptsDir}'.`);
    }
    const defaultConfig = {
        outdir: absOutDir,
        bundle: true,
    ...
    if (watch) {
        ...
        const scriptFiles = [];
        const restartEsbuild = debounce(() => restartEsbuildImpl({
            defaultConfig: {
 ...
 */
export function buildFlatEntryPointsMap(entryPointPaths) {
    const flatEntryPoints = {};
...
async function buildVisualizationMod({ absOutDir, srcDir, esbuildConfig: esbuildConfigPath, debug, watch, ...quiet }) {
    const stdout = mkStdout(quiet);
    const entryPoint = path.join(srcDir, "main.ts");
    if (!fse.existsSync(entryPoint)) {
        throw new Error(`Could not find entry point for visualization mod at '${entryPoint}'.`);
    }
    const defaultConfig = {
        outdir: absOutDir,
        bundle: true,
