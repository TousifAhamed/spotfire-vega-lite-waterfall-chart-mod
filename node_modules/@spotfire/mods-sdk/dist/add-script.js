...
import { mkStdout, readManifest, toAlphaNumWithSeparators, toTypeName, writeManifest, } from "./utils.js";
export async function addScript(id, { manifestPath: _manifestPath, scripts: _scripts, name, ...quiet }) {
    ...
    if (fse.existsSync(filePath)) {
        ...
        while (answer !== "yes" && answer !== "no") {
            answer = await rl.question(`The mod already contains a script file at '${filePath}'.\nDo you want to keep this file? [yes/no]\n`);
        }
        if (answer === "yes") {
            let i = 2;
            let newName;
            while (!newName || fse.existsSync(newName)) {
                newName = `${filePath.substring(0, filePath.length - 3)}(${i++}).ts`;
            }
